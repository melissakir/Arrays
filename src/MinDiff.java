...
public class MinDiff {
    ...
    public static int minDiff(int[] stones) {
        ...
        int newDiff;// variable for difference between consecutive integers
        for (int i =0; i < stones.length-1; i++){ //for all elements from the first to second to last
            newDiff = Math.abs(stones[i] - stones[i+1]); // positive distance between this element and the next
            if (newDiff < minDiff){ // compare new difference to the smallest difference so far
                minDiff = newDiff; // if smaller, make this the new smallest difference
            }
        }
